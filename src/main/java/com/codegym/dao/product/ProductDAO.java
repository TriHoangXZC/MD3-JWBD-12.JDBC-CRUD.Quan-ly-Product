...
public class ProductDAO implements IProductDAO{
    ...
    public static final String SQL_INSERT_INTO_PRODUCT = "INSERT INTO products (name, price, description, category_id) values (?, ?, ?, ?)";
    ...
    public Product findById(int id) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                String description = resultSet.getString("description");
                int categoryId = resultSet.getInt("category_id");
                product = new Product(id, name, price, description, categoryId);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public boolean deleteById(int id) {
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(SQL_DELETE_PRODUCT_BY_ID);
            preparedStatement.setInt(1, id);
            return preparedStatement.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public List<Product> findAllProductByName(String name) {
        ...
        try {
            ...
            while (resultSet.next()) {
                int id = resultSet.getInt("id");
                String nameFromSql = resultSet.getString("name");
                double price = resultSet.getDouble("price");
                String description = resultSet.getString("description");
                int categoryId = resultSet.getInt("category_id");
                Product product = new Product(id, nameFromSql, price, description, categoryId);
                products.add(product);
            }
        } catch (SQLException e) {
            e.printStackTrace();
